...
export const createComment = async ({ postId, userId, name, email, body }) => {
  const [result] = await pool.query(
    'INSERT INTO comments (postId, userId, name, email, body) VALUES (?, ?, ?, ?, ?)',
    [postId, userId, name, email ?? null, body]
  );
  return { id: result.insertId, postId, userId, name, email, body };
};
